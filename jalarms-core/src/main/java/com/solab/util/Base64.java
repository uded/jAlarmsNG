...
public class Base64 {
	 ...
	 * 
	 * @param b64
	 *            La cadena codificada. Puede estar separada en renglones.
	 * @return El arreglo de bytes resultante de haber decodificado la cadena.
	 */
	public static byte[] base64Decode(final String b64) {

		final String input = b64.trim();
		if (input.length() % 4 != 0) {
			throw new IllegalArgumentException("Incomplete data");
		}
		final ByteArrayOutputStream bout = new ByteArrayOutputStream(input.length() * 3 / 4 + 1);
	 ...
	 * 
	 * @param input
	 *            El arreglo de bytes a codificar.
	 * @param start
	 *            La posicion inicial donde se comienza a codificar
	 * @param len
	 *            El numero de bytes a codificar.
	 * @return El arreglo de bytes codificados en base 64, como una cadena.
	 */
	public static String base64Encode(final byte[] input, final int start, final int len) {
		final StringBuilder buf = new StringBuilder(input.length * 4 / 3);
	 ...
	 * 
	 * @param c
	 *            El caracter cuya posicion se desea saber.
	 * @return El indice del caracter en el alfabeto base64, o -1 si no esta.
	 */
	private static int indice(final char c) {
