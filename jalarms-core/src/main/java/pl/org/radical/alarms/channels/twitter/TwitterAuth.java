...
public class TwitterAuth {
	...
	private static Map<String, String> parseResponse(final String resp) {
		final String[] partes = resp.split("&");
		final HashMap<String, String> map = new HashMap<String, String>();
		for (final String p : partes) {
			final int pos = p.indexOf('=');
			final String k = p.substring(0, pos);
			final String v = p.substring(pos + 1);
			try {
	...
	public static void main(final String[] args) throws Exception {
		final TwitterChannel chan = new TwitterChannel();
		chan.init();
		chan.setTokenSecret("");
		...
		final String secreto = datos.get("oauth_token_secret");
		if (secreto == null) {
		...
		//
		final String token1 = datos.get("oauth_token");
		System.out.println();
		...
		System.out.println();
		System.out
		        .println("In case you're using Spring, here's the XML you need to paste into your application context (inside the bean definition of the TwitterChannel):");
